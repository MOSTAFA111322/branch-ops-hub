...
import { Download, Filter, Save, Search, UsersRound } from "lucide-react";
...
export function UserAccountManager() {
  ...
  const choose = (id: number) => { const item = users.find((row) => row.id === id); if (!item) return; setSelectedId(id); setName(item.name ?? ""); setRole(item.role as Role); setBranchId(item.branchId ? String(item.branchId) : ""); setRegionId(item.regionId ? String(item.regionId) : ""); setIsActive(item.isActive !== false); setCanExportAuditLogs(item.canExportAuditLogs === true); };
  const [isActive, setIsActive] = useState(true);
  const [canExportAuditLogs, setCanExportAuditLogs] = useState(false);
  const save = () => { if (!selectedId) return; update.mutate({ userId: selectedId, name: name.trim() || undefined, role, branchId: branchId ? Number(branchId) : null, regionId: regionId ? Number(regionId) : null, isActive, canExportAuditLogs }); };
  const exportStatusHistory = () => {
    const rows = (statusAudit.data ?? []).filter((row) => row.action === "update").map((row) => ({
      "معرّف السجل": row.id,
      "معرّف الحساب": row.entityId ?? "—",
      "المنفذ": row.actor?.name ?? "مدير النظام",
      "الفرع": row.branch?.name ?? "—",
      "تاريخ التغيير": new Date(row.createdAt).toLocaleString("ar-SA"),
      "البيانات السابقة": row.beforeData ?? "—",
      "البيانات اللاحقة": row.afterData ?? "—",
    }));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(rows.length ? rows : [{ "ملاحظة": "لا توجد تغييرات مسجلة" }]), "سجل الحالة");
    XLSX.writeFile(workbook, `سجل-حالة-الحسابات-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
