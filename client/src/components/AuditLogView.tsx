...
export function AuditLogView() {
  ...
  const scheduledReports = useMemo(() => rows.filter((row) => row.entityType === "scheduled_report"), [rows]);
  const latestReport = scheduledReports[0];
  const latestReportData = useMemo(() => { if (!latestReport?.afterData) return null; try { return JSON.parse(latestReport.afterData) as { marker?: string; delivered?: boolean; recipientRoles?: string[]; recipientIds?: number[]; taskUid?: string }; } catch { return null; } }, [latestReport]);
  return <div className="space-y-4" dir="rtl">
    ...
    <Card className="rounded-2xl border-[#dfe9df] bg-[#fbfdfb] shadow-sm"><CardHeader><CardTitle className="text-base">مراجعة التقرير الشهري المجدول</CardTitle><p className="mt-1 text-xs text-[#89948b]">تُعرض آخر عملية مسجلة فقط؛ عدم وجود سجل يعني أن المهمة لم تسجل تنفيذًا بعد.</p></CardHeader><CardContent>{!latestReport ? <p className="text-xs text-[#89948b]">لا يوجد تنفيذ مسجل حتى الآن.</p> : <div className="grid gap-3 md:grid-cols-4"><div><p className="text-[10px] text-[#89948b]">الفترة</p><p className="mt-1 text-sm font-bold text-[#174c3d]">{latestReportData?.marker?.replace("monthly-financial-report:", "") ?? "غير محددة"}</p></div><div><p className="text-[10px] text-[#89948b]">الحالة</p><Badge className={`mt-1 rounded-full text-[10px] ${latestReportData?.delivered ? "bg-[#e2f3e7] text-[#2d7d58]" : "bg-[#fff0dc] text-[#ad7a27]"}`}>{latestReportData?.delivered ? "تم التسليم" : "تم التسجيل"}</Badge></div><div><p className="text-[10px] text-[#89948b]">المستلمون</p><p className="mt-1 text-xs font-semibold">{latestReportData?.recipientIds?.length ?? 0} مستخدم</p></div><div><p className="text-[10px] text-[#89948b]">آخر تشغيل</p><p className="mt-1 text-xs font-semibold">{new Date(latestReport.createdAt).toLocaleString("ar-SA")}</p></div></div>}</CardContent></Card>
