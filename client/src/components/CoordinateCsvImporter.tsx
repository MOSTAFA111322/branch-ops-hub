...
export function CoordinateCsvImporter() {
  ...
  const auditExportRows = () => filteredAudit.map((entry) => ({ "التاريخ": new Date(entry.createdAt).toLocaleString("ar-SA"), "المستخدم": entry.actor?.name ?? "النظام", "الفرع": entry.branch?.name ?? "عدة فروع", "العملية": entry.action === "bulk_verify" ? "اعتماد جماعي" : "تعديل", "قبل التعديل": entry.beforeData ?? "—", "بعد التعديل": entry.afterData ?? "—" }));
  const exportAuditExcel = () => { const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(auditExportRows()), "سجل الإحداثيات"); XLSX.writeFile(workbook, "سجل-تغييرات-إحداثيات-الفروع.xlsx"); };
  const exportAuditPdf = () => { const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "a4" }); doc.setFontSize(15); doc.text("Branch Coordinates Audit Log", 40, 38); doc.setFontSize(9); doc.text(`Filtered records: ${filteredAudit.length}`, 40, 56); let y = 78; auditExportRows().slice(0, 30).forEach((row, index) => { const line = `${index + 1}. ${row["التاريخ"]} | ${row["المستخدم"]} | ${row["الفرع"]} | ${row["العملية"]}`; doc.text(line.slice(0, 145), 40, y); y += 16; }); doc.save("سجل-تغييرات-إحداثيات-الفروع.pdf"); };
