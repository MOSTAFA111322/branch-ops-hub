...
export function CoordinateCsvImporter() {
  ...
  const mutation = trpc.branches.importCoordinates.useMutation({ onSuccess: (result) => { setMessage(`تم تحديث ${result.updated} فرع، ورفض ${result.rejected} صف.`); branchesQuery.refetch(); }, onError: (error) => setMessage(error.message) });
  const verifyMutation = trpc.branches.verifyCoordinates.useMutation({ onSuccess: (result) => { setMessage(`تم توثيق ${result.verified} إحداثية جماعيًا، وتعذر توثيق ${result.rejected}.`); branchesQuery.refetch(); }, onError: (error) => setMessage(error.message) });
