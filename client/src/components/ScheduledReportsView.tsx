...
export function ScheduledReportsView({ canRetry = false }: { canRetry?: boolean }) {
  ...
  const readyReportDateRangeInvalid = Boolean(readyReportDateFrom && readyReportDateTo && readyReportDateFrom > readyReportDateTo);
  ...
  const exportReadyReportsCsv = () => {
    ...
    const rows = sortedReadyReports.map((report) => [
      report.id,
...
<Input type="date" value={readyReportDateFrom} onChange={(event) => setReadyReportDateFrom(event.target.value)} className="h-9 min-w-0 rounded-lg text-xs" aria-label="من تاريخ سجل التقارير" /><Input type="date" value={readyReportDateTo} onChange={(event) => setReadyReportDateTo(event.target.value)} className="h-9 min-w-0 rounded-lg text-xs" aria-label="إلى تاريخ سجل التقارير" /></div></div>{readyReportDateRangeInvalid && <p role="alert" className="mb-3 text-[11px] text-[#a45b4e]">تحقق من نطاق التاريخ: يجب أن يسبق تاريخ البداية تاريخ النهاية.</p>}{(readyReportDateFrom || readyReportDateTo) && <p className="mb-3 text-[10px] text-[#6b8172]">سيشمل ملف CSV التقارير المطابقة للتاريخ المحدد وبقية الفلاتر الحالية: {sortedReadyReports.length} تقرير.</p>}
