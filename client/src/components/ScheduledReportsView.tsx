...
import { AlertTriangle, CheckCircle2, Clock3, FileText, Play, Pause, RefreshCw, Download, RotateCcw, Send, Trash2 } from "lucide-react";
...
export function ScheduledReportsView({ canRetry = false }: { canRetry?: boolean }) {
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      <Card className="border-[#dfe9df] bg-white shadow-[0_5px_18px_rgba(39,70,48,0.04)]" aria-labelledby="ready-reports-title"><CardHeader><div className="flex items-center justify-between gap-2"><div><CardTitle id="ready-reports-title" className="flex items-center gap-2 text-base"><FileText className="h-4 w-4 text-[#4d8068]" /> التقارير الجاهزة للمشاركة</CardTitle><p className="mt-1 text-xs text-[#89948b]">تُنشأ الجدولة التقرير وتحفظه هنا. المشاركة عبر واتساب العادي يدوية: يفتح النظام المحادثة والرسالة، ثم يضغط المستخدم إرسال ويرفق PDF عند الحاجة.</p></div><Badge variant="outline" className="rounded-full text-[10px]">{readyReports.length} تقرير</Badge></div></CardHeader><CardContent>{readyReports.length === 0 ? <p className="rounded-xl bg-[#f7faf7] p-4 text-center text-xs text-[#89948b]">لا توجد تقارير جاهزة للمشاركة بعد.</p> : <div className="space-y-2">{readyReports.slice(0, 8).map((report) => <div key={report.id} className="flex flex-col gap-2 rounded-xl border border-[#edf1ed] bg-[#fbfdfb] p-3 md:flex-row md:items-center md:justify-between"><div className="min-w-0"><p className="truncate text-xs font-semibold text-[#1c2820]">{report.title || "تقرير تنفيذي مالي"}</p><p className="mt-1 text-[10px] text-[#89948b]">الفترة: {report.period} · المركز: {report.costCenter === "all" ? "كل المراكز" : report.costCenter} · الرقم: {report.phone}</p><p className="mt-1 text-[10px] text-[#4d8068]">{report.status} · {new Date(report.createdAt).toLocaleString("ar-SA")}</p></div><Button size="sm" variant="outline" className="gap-1 rounded-xl text-[11px]" onClick={() => window.open(`https://wa.me/${report.phone}?text=${encodeURIComponent(`تقرير: ${report.title || "التقرير التنفيذي المالي"}\\nالفترة: ${report.period}\\nالتقرير جاهز للمراجعة والمشاركة اليدوية.`)}`, "_blank", "noopener,noreferrer")}><Send className="h-3.5 w-3.5" />فتح واتساب</Button></div>)}</div>} {readyReports.length > 0 && <Button variant="ghost" size="sm" className="mt-2 gap-1 text-[11px] text-[#89948b]" onClick={() => { localStorage.removeItem("branch-ops-ready-reports"); setReadyReports([]); }}><Trash2 className="h-3.5 w-3.5" />مسح السجل المحلي</Button>}</CardContent></Card>
