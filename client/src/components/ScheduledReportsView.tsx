...
export function ScheduledReportsView({ canRetry = false }: { canRetry?: boolean }) {
  ...
  const deliveryMarker = useMemo(() => {
    ...
    for (const log of matching) {
      try { const marker = JSON.parse(log.afterData ?? "{}").marker; if (typeof marker === "string") return marker; } catch { /* ignore malformed audit rows */ }
    }
    return "";
