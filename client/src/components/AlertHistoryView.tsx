...
export function AlertHistoryView() {
  ...
  const alerts = useMemo(() => [...((summary?.alerts ?? []) as AlertItem[]), ...(checklistAlerts as AlertItem[])].map((item) => ({ ...item, id: String(item.id) })), [summary, checklistAlerts]);
  const markRead = (id: string) => { const next = Array.from(new Set([...readIds, id])); setReadIds(next); localStorage.setItem(storageKey, JSON.stringify(next)); };
  const markAllRead = () => { const next = alerts.map((item) => item.id); setReadIds(next); localStorage.setItem(storageKey, JSON.stringify(next)); };
