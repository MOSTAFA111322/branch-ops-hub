...
function OperationsView({ section, user, onNavigate }: { section: string; user?: { role?: string } | null; onNavigate: (section: string) => void }) {
  ...
  const updateRecordStatus = async (id: number, status: string, extra?: { score?: number; notes?: string; reportTitle?: string; findings?: string; recommendations?: string; approvalStatus?: "draft" | "submitted" | "approved"; priority?: "low" | "medium" | "high" | "urgent"; dueAt?: Date; version?: string; expiresAt?: Date; documentStatus?: "valid" | "expiring" | "expired" | "missing"; documentType?: string; fileUrl?: string; rootCause?: string; ticketType?: "breakdown" | "preventive" | "warranty"; warrantyUntil?: Date }) => {
    try {
      if (section === "الإجراءات والتحسين") await actionStatus.mutateAsync({ id, status: status as "open" | "in_progress" | "pending_review" | "closed", priority: extra?.priority as "low" | "medium" | "high" | "urgent" | undefined, dueAt: extra?.dueAt });
      ...
      else if (section === "الزيارات والفحص") await visitStatus.mutateAsync({ id, status: status as "scheduled" | "in_progress" | "completed" | "cancelled", score: extra?.score, notes: extra?.notes, reportTitle: extra?.reportTitle, findings: extra?.findings, recommendations: extra?.recommendations, approvalStatus: extra?.approvalStatus });
      else if (section === "الجودة والشكاوى") await qualityStatus.mutateAsync({ id, status: status as "open" | "investigating" | "resolved" | "closed", rootCause: extra?.rootCause });
      else if (section === "الصيانة والأصول") await maintenanceStatus.mutateAsync({ id, status: status as "open" | "assigned" | "in_progress" | "resolved" | "closed", ticketType: extra?.ticketType as "breakdown" | "preventive" | "warranty" | undefined, priority: extra?.priority as "low" | "medium" | "high" | "urgent" | undefined, warrantyUntil: extra?.warrantyUntil });
