...
function OperationsView({ section, user, onNavigate }: { section: string; user?: { role?: string } | null; onNavigate: (section: string) => void }) {
  ...
  const submitRecord = async () => {
    ...
    try {
      ...
      await utils.ops.overview.invalidate();
      setDraftTitle(""); setDraftBranchId(""); setCreateError(""); setCreateOpen(false);
  ...
  return <section className="mb-6 rounded-3xl border border-[#dfe9df] bg-white p-5 shadow-[0_8px_25px_rgba(39,70,48,0.05)] md:p-6">
    ...
    {createOpen && <div className="mt-5 rounded-2xl border border-[#dce9df] bg-[#f7fbf8] p-4"><div className="grid gap-3 md:grid-cols-[1fr_220px_auto]"><Input value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} placeholder="عنوان السجل أو وصفه المختصر" className="rounded-xl border-[#dce9df] bg-white" /><select value={draftBranchId} onChange={(event) => setDraftBranchId(event.target.value)} className="h-10 rounded-xl border border-[#dce9df] bg-white px-3 text-xs text-[#43544a]"><option value="">اختر الفرع</option>{branchOptions.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select><Button onClick={submitRecord} className="rounded-xl bg-[#24724e]">حفظ السجل</Button></div>{createError && <p className="mt-2 text-xs text-[#a25c3d]">{createError}</p>}</div>}
