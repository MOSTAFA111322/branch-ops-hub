...
function OperationsView({ section, user, onNavigate }: { section: string; user?: { role?: string } | null; onNavigate: (section: string) => void }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return (rows as Array<Record<string, unknown>>).filter((row) => Object.values(row).some((value) => String(value ?? "").toLowerCase().includes(query)));
  }, [filterText, rows]);
  ...
  const removeRecord = async (id: number, row: Record<string, unknown>) => {
    ...
    try {
      ...
      else if (section === "المهام والطلبات" && row.requestType) await requestRemove.mutateAsync({ id });
  ...
  const updateRecordStatus = async (id: number, status: string, extra?: { score?: number; notes?: string }) => {
    try {
      if (section === "الإجراءات والتحسين") await actionStatus.mutateAsync({ id, status: status as "open" | "in_progress" | "pending_review" | "closed" });
      else if (section === "الزيارات والفحص") await visitStatus.mutateAsync({ id, status: status as "scheduled" | "in_progress" | "completed" | "cancelled", score: extra?.score, notes: extra?.notes });
      else if (section === "الجودة والشكاوى") await qualityStatus.mutateAsync({ id, status: status as "open" | "investigating" | "resolved" | "closed" });
      else if (section === "الصيانة والأصول") await maintenanceStatus.mutateAsync({ id, status: status as "open" | "assigned" | "in_progress" | "resolved" | "closed" });
      else if (section === "المهام والطلبات") {
        const current = (filteredRows as Array<Record<string, unknown>>).find((row) => Number(row.id) === id);
        if (current?.requestType) await requestStatus.mutateAsync({ id, status: status as "new" | "assigned" | "in_progress" | "completed" | "rejected" });
        else await taskStatus.mutateAsync({ id, status: status as "todo" | "in_progress" | "done" });
      }
      await utils.ops.overview.invalidate();
    } catch (error) { setCreateError(error instanceof Error ? error.message : "تعذر تحديث حالة السجل."); }
  };
  const submitRecord = async () => {
    ...
    try {
      ...
      else if (section === "الوثائق والتراخيص") await documentCreate.mutateAsync({ branchId, title, documentType: "تشغيلي" });
      ...
      else if (section === "المهام والطلبات" && recordKind === "request") await requestCreate.mutateAsync({ branchId, title, requestType: "تشغيل", priority: "medium" });
      ...
      await utils.ops.overview.invalidate();
      setDraftTitle(""); setDraftBranchId(""); setRecordKind("task"); setCreateError(""); setCreateOpen(false);
