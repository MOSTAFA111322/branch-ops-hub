  ...
  const updateRecordStatus = async (id: number, status: string) => {
    try {
      if (section === "الإجراءات والتحسين") await actionStatus.mutateAsync({ id, status: status as "open" | "in_progress" | "pending_review" | "closed" });
        ...
        const current = (rows as Array<Record<string, unknown>>).find((row) => Number(row.id) === id);
