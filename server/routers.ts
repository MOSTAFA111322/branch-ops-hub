...
export const appRouter = router({
  ...
  usage: router({
    ...
    list: roleProcedure(["admin", "area_manager"]).query(async () => { const db = await getDb(); if (!db) return []; return db.select().from(auditLogs).where(eq(auditLogs.entityType, "command_usage")).limit(500); }),
  }),
  preferences: router({
