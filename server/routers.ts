...
export const appRouter = router({
  ...
  usage: router({
    log: protectedProcedure.input(z.object({ command: z.string().min(1).max(120), surface: z.enum(["shortcut", "palette", "quick_action"]) })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); await recordAudit(db, { actorId: ctx.user.id, entityType: "command_usage", action: input.surface, afterData: { command: input.command, role: ctx.user.role } }); return { success: true }; }),
    list: roleProcedure(["admin", "area_manager"]).input(z.object({ from: z.date().optional(), to: z.date().optional() }).optional()).query(async ({ input }) => { const db = await getDb(); if (!db) return { rows: [], summary: [], total: 0 }; const rows = await db.select().from(auditLogs).where(eq(auditLogs.entityType, "command_usage")).limit(1000); const filtered = rows.filter(row => (!input?.from || row.createdAt >= input.from) && (!input?.to || row.createdAt <= input.to)); const grouped = new Map<string, { command: string; count: number; lastUsed: Date }>(); for (const row of filtered) { let command = "غير محدد"; try { command = JSON.parse(row.afterData ?? "{}").command ?? command; } catch { /* سجل قديم */ } const existing = grouped.get(command); if (existing) { existing.count += 1; if (row.createdAt > existing.lastUsed) existing.lastUsed = row.createdAt; } else grouped.set(command, { command, count: 1, lastUsed: row.createdAt }); } return { rows: filtered.reverse(), summary: Array.from(grouped.values()).sort((a, b) => b.count - a.count), total: filtered.length }; }),
  }),
  preferences: router({
