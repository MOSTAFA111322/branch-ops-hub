...
import { getBranchById, getBranchProfile, canAccessBranch, getDashboardSummary, getInventoryMovementAnalysis, getOperationsOverview, listBranches, getDb, listFavoritePeriodRanges, saveFavoritePeriodRange, deleteFavoritePeriodRange, renameFavoritePeriodRange, reorderFavoritePeriodRanges, updateFavoritePeriodSettings } from "./db";
...
export const appRouter = router({
  ...
  exports: router({
    log: protectedProcedure.input(z.object({ format: z.enum(["csv", "excel", "pdf"]), report: z.string().trim().min(1).max(120), rowCount: z.number().int().min(0).max(100000).optional(), status: z.enum(["started", "success", "failed"]), error: z.string().max(500).optional(), filters: z.record(z.string(), z.unknown()).optional() })).mutation(async ({ ctx, input }) => {
      ...
      if (input.status === "failed") {
        const [preference] = await db.select({ threshold: dashboardPreferences.exportFailureThreshold }).from(dashboardPreferences).where(eq(dashboardPreferences.userId, ctx.user.id)).limit(1);
        const threshold = Math.min(100, Math.max(1, Number(preference?.threshold ?? 20)));
        const recent = await db.select({ action: auditLogs.action, createdAt: auditLogs.createdAt, afterData: auditLogs.afterData }).from(auditLogs).where(eq(auditLogs.entityType, "data_export")).orderBy(desc(auditLogs.createdAt)).limit(100);
        const failed = recent.filter((row) => row.action === "export_failed").length;
        const failureRate = recent.length ? (failed / recent.length) * 100 : 0;
        const recentAlert = await db.select({ id: auditLogs.id, createdAt: auditLogs.createdAt }).from(auditLogs).where(and(eq(auditLogs.entityType, "export_failure_alert"), eq(auditLogs.action, "sent"))).orderBy(desc(auditLogs.createdAt)).limit(1);
        const alertAlreadySent = recentAlert[0] && Date.now() - new Date(recentAlert[0].createdAt).getTime() < 24 * 60 * 60 * 1000;
        if (failed >= 3 && failureRate >= threshold && !alertAlreadySent) {
          const content = `بلغ معدل فشل عمليات التصدير ${failureRate.toFixed(1)}% خلال آخر ${recent.length} عملية، متجاوزًا الحد المضبوط ${threshold}%. يرجى مراجعة سجل الأخطاء والصلاحيات.`;
          const delivered = await notifyOwner({ title: "تنبيه ارتفاع فشل التصدير", content });
          const admins = await db.select({ id: users.id }).from(users).where(inArray(users.role, ["admin", "area_manager"]));
          if (admins.length) await db.insert(notifications).values(admins.map((admin) => ({ recipientId: admin.id, kind: "export_failure_rate_high", title: "تنبيه ارتفاع فشل التصدير", content, entityType: "export_failure_alert" })));
          await recordAudit(db, { actorId: ctx.user.id, entityType: "export_failure_alert", action: "sent", afterData: { threshold, failureRate, sampleSize: recent.length, delivered } });
        }
      }
      return { success: true };
    }),
  ...
  financials: router({
    list: protectedProcedure.input(z.object({ branchId: z.number().int().positive().optional(), year: z.number().int().min(2000).max(2200).optional(), month: z.number().int().min(1).max(12).optional() }).optional()).query(async ({ input, ctx }) => {
      ...
      if (input?.branchId !== undefined && !allowedIds.includes(input.branchId)) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية عرض بيانات هذا الفرع" });
      const rows = await db.select().from(branchFinancialSnapshots).where(inArray(branchFinancialSnapshots.branchId, input?.branchId !== undefined ? [input.branchId] : allowedIds));
  ...
  actions: router({
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(correctiveActions).where(eq(correctiveActions.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "الإجراء غير موجود" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.delete(correctiveActions).where(eq(correctiveActions.id, input.id)); await recordAudit(db, { actorId: ctx.user.id, branchId: current.branchId, entityType: "corrective_action", entityId: current.id, action: "delete", beforeData: current }); return { success: true }; }),
    updateStatus: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), status: z.enum(["open", "in_progress", "pending_review", "closed"]), closureEvidenceUrl: z.string().url().optional(), priority: z.enum(["low", "medium", "high", "urgent"]).optional(), dueAt: z.date().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(correctiveActions).where(eq(correctiveActions.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "الإجراء غير موجود" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); const patch = { status: input.status, closureEvidenceUrl: input.closureEvidenceUrl, priority: input.priority, dueAt: input.dueAt }; await db.update(correctiveActions).set(patch).where(eq(correctiveActions.id, input.id)); await recordAudit(db, { actorId: ctx.user.id, branchId: current.branchId, entityType: "corrective_action", entityId: current.id, action: "status_update", beforeData: current, afterData: patch }); return { success: true }; }),
    create: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({
      ...
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      if (!(await canAccessBranch(ctx.user, input.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" });
      const result = await db.insert(correctiveActions).values({ ...input, ownerId: ctx.user.id });
      return { id: result[0].insertId };
    }),
    update: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), title: z.string().min(1).max(220).optional(), description: z.string().optional(), ownerId: z.number().int().positive().nullable().optional(), priority: z.enum(["low", "medium", "high", "urgent"]).optional(), dueAt: z.date().optional(), closureEvidenceUrl: z.string().url().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(correctiveActions).where(eq(correctiveActions.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "الإجراء غير موجود" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); const patch = { title: input.title, description: input.description, ownerId: input.ownerId, priority: input.priority, dueAt: input.dueAt, closureEvidenceUrl: input.closureEvidenceUrl }; await db.update(correctiveActions).set(patch).where(eq(correctiveActions.id, input.id)); await recordAudit(db, { actorId: ctx.user.id, branchId: current.branchId, entityType: "corrective_action", entityId: current.id, action: "update", beforeData: current, afterData: patch }); return { success: true }; }),
  }),
  visits: router({
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [visit] = await db.select().from(visits).where(eq(visits.id, input.id)).limit(1); if (!visit) throw new TRPCError({ code: "NOT_FOUND", message: "الزيارة غير موجودة" }); if (!(await canAccessBranch(ctx.user, visit.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.delete(visits).where(eq(visits.id, input.id)); return { success: true }; }),
    updateStatus: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), status: z.enum(["scheduled", "in_progress", "completed", "cancelled"]), score: z.number().min(0).max(100).optional(), notes: z.string().min(1).optional(), reportTitle: z.string().min(1).max(220).optional(), findings: z.string().optional(), recommendations: z.string().optional(), approvalStatus: z.enum(["draft", "submitted", "approved"]).optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [visit] = await db.select().from(visits).where(eq(visits.id, input.id)).limit(1); if (!visit) throw new TRPCError({ code: "NOT_FOUND", message: "الزيارة غير موجودة" }); if (!(await canAccessBranch(ctx.user, visit.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.update(visits).set({ status: input.status, score: input.score?.toString(), notes: input.notes, reportTitle: input.reportTitle, findings: input.findings, recommendations: input.recommendations, approvalStatus: input.approvalStatus, approvedAt: input.approvalStatus === "approved" ? new Date() : undefined, completedAt: input.status === "completed" ? new Date() : undefined }).where(eq(visits.id, input.id)); if (input.status === "completed" && input.score !== undefined) await db.update(branches).set({ healthScore: input.score.toString() }).where(eq(branches.id, visit.branchId)); return { success: true, branchId: visit.branchId, score: input.score ?? null, approvalStatus: input.approvalStatus ?? visit.approvalStatus }; }),
    update: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), scheduledAt: z.date().optional(), notes: z.string().optional(), reportTitle: z.string().min(1).max(220).optional(), findings: z.string().optional(), recommendations: z.string().optional(), checklistTemplateId: z.number().int().positive().nullable().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select({ id: visits.id, branchId: visits.branchId }).from(visits).where(eq(visits.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "الزيارة غير موجودة" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.update(visits).set({ scheduledAt: input.scheduledAt, notes: input.notes, reportTitle: input.reportTitle, findings: input.findings, recommendations: input.recommendations, checklistTemplateId: input.checklistTemplateId }).where(eq(visits.id, input.id)); return { success: true }; }),
    create: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ branchId: z.number().int().positive(), scheduledAt: z.date().optional(), notes: z.string().optional(), checklistTemplateId: z.number().int().positive().optional() })).mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      if (!(await canAccessBranch(ctx.user, input.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" });
    ...
    bulkUpdate: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ ids: z.array(z.number().int().positive()).min(1).max(100), status: z.enum(["scheduled", "in_progress", "completed", "cancelled"]), approvalStatus: z.enum(["draft", "submitted", "approved"]).optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const rows = await db.select().from(visits).where(inArray(visits.id, input.ids)); if (rows.length !== input.ids.length) throw new TRPCError({ code: "NOT_FOUND", message: "بعض الزيارات غير موجودة" }); if (!(await Promise.all(rows.map((visit) => canAccessBranch(ctx.user, visit.branchId))).then((allowed) => allowed.every(Boolean)))) throw new TRPCError({ code: "FORBIDDEN", message: "تتضمن القائمة زيارة خارج نطاق صلاحياتك" }); const patch = { status: input.status, ...(input.approvalStatus ? { approvalStatus: input.approvalStatus } : {}), ...(input.status === "completed" ? { completedAt: new Date() } : {}) }; await db.update(visits).set(patch).where(inArray(visits.id, input.ids)); await Promise.all(rows.map(visit => recordAudit(db, { actorId: ctx.user.id, branchId: visit.branchId, entityType: "visit", entityId: visit.id, action: "bulk_update", beforeData: visit, afterData: patch }))); return { success: true, count: rows.length }; }),
  }),
  documents: router({
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [document] = await db.select().from(documents).where(eq(documents.id, input.id)).limit(1); if (!document) throw new TRPCError({ code: "NOT_FOUND", message: "الوثيقة غير موجودة" }); if (!(await canAccessBranch(ctx.user, document.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.delete(documents).where(eq(documents.id, input.id)); return { success: true }; }),
    update: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), documentType: z.string().min(1).max(100).optional(), version: z.string().min(1).max(32).optional(), expiresAt: z.date().optional(), status: z.enum(["valid", "expiring", "expired", "missing"]).optional(), fileUrl: z.string().url().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(documents).where(eq(documents.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "الوثيقة غير موجودة" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); const next = { documentType: input.documentType ?? current.documentType, version: input.version ?? current.version, expiresAt: input.expiresAt ?? current.expiresAt, status: input.status ?? current.status, fileUrl: input.fileUrl ?? current.fileUrl }; await db.update(documents).set(next).where(eq(documents.id, input.id)); await db.insert(documentVersions).values({ documentId: input.id, ...next, recordedBy: ctx.user.id }); return { success: true }; }),
  ...
  quality: router({
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(qualityCases).where(eq(qualityCases.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "حالة الجودة غير موجودة" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.delete(qualityCases).where(eq(qualityCases.id, input.id)); return { success: true }; }),
    updateStatus: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ id: z.number().int().positive(), status: z.enum(["open", "investigating", "resolved", "closed"]), rootCause: z.string().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select().from(qualityCases).where(eq(qualityCases.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "حالة الجودة غير موجودة" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.update(qualityCases).set({ status: input.status, rootCause: input.rootCause }).where(eq(qualityCases.id, input.id)); return { success: true }; }),
    create: roleProcedure(["admin", "area_manager", "branch_manager", "quality"]).input(z.object({ branchId: z.number().int().positive(), title: z.string().min(1).max(220), caseType: z.enum(["non_conformity", "complaint", "observation"]), severity: z.enum(["low", "medium", "high", "critical"]).default("medium"), description: z.string().optional() })).mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      if (!(await canAccessBranch(ctx.user, input.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" });
  ...
  maintenance: router({
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [ticket] = await db.select().from(maintenanceTickets).where(eq(maintenanceTickets.id, input.id)).limit(1); if (!ticket) throw new TRPCError({ code: "NOT_FOUND", message: "بلاغ الصيانة غير موجود" }); if (!(await canAccessBranch(ctx.user, ticket.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.delete(maintenanceTickets).where(eq(maintenanceTickets.id, input.id)); return { success: true }; }),
    updateStatus: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ id: z.number().int().positive(), status: z.enum(["open", "assigned", "in_progress", "resolved", "closed"]), ticketType: z.enum(["breakdown", "preventive", "warranty"]).optional(), priority: z.enum(["low", "medium", "high", "urgent"]).optional(), warrantyUntil: z.date().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [ticket] = await db.select().from(maintenanceTickets).where(eq(maintenanceTickets.id, input.id)).limit(1); if (!ticket) throw new TRPCError({ code: "NOT_FOUND", message: "بلاغ الصيانة غير موجود" }); if (!(await canAccessBranch(ctx.user, ticket.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.update(maintenanceTickets).set({ status: input.status, ticketType: input.ticketType, priority: input.priority, warrantyUntil: input.warrantyUntil }).where(eq(maintenanceTickets.id, input.id)); return { success: true }; }),
    create: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ branchId: z.number().int().positive(), assetName: z.string().min(1).max(160), title: z.string().min(1).max(220), ticketType: z.enum(["breakdown", "preventive", "warranty"]).default("breakdown"), priority: z.enum(["low", "medium", "high", "urgent"]).default("medium") })).mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      if (!(await canAccessBranch(ctx.user, input.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" });
    ...
    update: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ id: z.number().int().positive(), assetName: z.string().min(1).max(160).optional(), title: z.string().min(1).max(220).optional(), ticketType: z.enum(["breakdown", "preventive", "warranty"]).optional(), priority: z.enum(["low", "medium", "high", "urgent"]).optional(), warrantyUntil: z.date().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select({ id: maintenanceTickets.id, branchId: maintenanceTickets.branchId }).from(maintenanceTickets).where(eq(maintenanceTickets.id, input.id)).limit(1); if (!current) throw new TRPCError({ code: "NOT_FOUND", message: "بلاغ الصيانة غير موجود" }); if (!(await canAccessBranch(ctx.user, current.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" }); await db.update(maintenanceTickets).set({ assetName: input.assetName, title: input.title, ticketType: input.ticketType, priority: input.priority, warrantyUntil: input.warrantyUntil }).where(eq(maintenanceTickets.id, input.id)); return { success: true }; }),
  }),
  assets: router({
    list: protectedProcedure.input(z.object({ branchId: z.number().int().positive() })).query(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) return [];
      if (!(await canAccessBranch(ctx.user, input.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية عرض أصول هذا الفرع" });
    ...
    update: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ id: z.number().int().positive(), name: z.string().min(1).max(180).optional(), assetType: z.string().min(1).max(100).optional(), serialNumber: z.string().max(120).optional(), status: z.enum(["active", "maintenance", "retired"]).optional(), warrantyUntil: z.date().optional() })).mutation(async ({ input, ctx }) => {
      ...
      if (!(await canAccessBranch(ctx.user, asset.branchId))) throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية هذا الفرع" });
    ...
    remove: roleProcedure(["admin", "area_manager", "branch_manager", "maintenance"]).input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => {
      const db = await getDb();
      if (!db) throw new Error("Database unavailable");
      const [asset] = await db.select().from(branchAssets).where(eq(branchAssets.id, input.id)).limit(1);
      if (!asset) throw new TRPCError({ code: "NOT_FOUND", message: "الأصل غير موجود" });
  ...
  preferences: router({
    getDashboard: protectedProcedure.query(async ({ ctx }) => { const db = await getDb(); if (!db) return { visibleWidgets: null }; const [row] = await db.select().from(dashboardPreferences).where(eq(dashboardPreferences.userId, ctx.user.id)).limit(1); return { visibleWidgets: row ? JSON.parse(row.visibleWidgets) : null, exportFailureThreshold: row?.exportFailureThreshold ?? 20 }; }),
    getExportFailureThreshold: protectedProcedure.query(async ({ ctx }) => { const db = await getDb(); if (!db) return { threshold: 20 }; const [row] = await db.select({ threshold: dashboardPreferences.exportFailureThreshold }).from(dashboardPreferences).where(eq(dashboardPreferences.userId, ctx.user.id)).limit(1); return { threshold: row?.threshold ?? 20 }; }),
    saveExportFailureThreshold: roleProcedure(["admin", "area_manager"]).input(z.object({ threshold: z.number().int().min(1).max(100) })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const [current] = await db.select({ id: dashboardPreferences.id, visibleWidgets: dashboardPreferences.visibleWidgets }).from(dashboardPreferences).where(eq(dashboardPreferences.userId, ctx.user.id)).limit(1); if (current) await db.update(dashboardPreferences).set({ exportFailureThreshold: input.threshold }).where(eq(dashboardPreferences.userId, ctx.user.id)); else await db.insert(dashboardPreferences).values({ userId: ctx.user.id, visibleWidgets: JSON.stringify(["overview"]), exportFailureThreshold: input.threshold }); await recordAudit(db, { actorId: ctx.user.id, entityType: "dashboard_preferences", action: "export_failure_threshold_updated", afterData: input }); return { success: true, threshold: input.threshold }; }),
